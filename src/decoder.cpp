...
// 启动解码线程
// ---------------------------------------------------------------------------
// ★ M8 阶段修正：这里【不再】顺手调用 queue_->start()。
//
//   参考工程原来是这么写的，看起来很方便，但实测撞出了两个问题：
//
//   ① 队列的生死绑在了 Decoder 上。如果解码器打开失败（openComponent
//      直接 return），start() 就永远不会被调用，队列一直停在终止态，
//      readThread 投进来的包会被【静默丢弃】，而且没有任何错误提示。
//      我们在 M8 实验里正好撞上了这个现象：队列里 0 个包、serial 还是 0。
//
//   ② PacketQueue::start() 【不幂等】—— 它每次都会压入一个新的 flush
//      标记并把 serial 加一。所以"两处都可能调用它"是很危险的，
//      一旦重复调用就会凭空多出一个 flush 边界、serial 也会多跳一次。
//
//   修正后的职责划分（更符合直觉）：
//     · 队列由 FFPlayer 创建 → 就该由 FFPlayer 启动（见 openComponent）
//     · Decoder::start() 只负责"起线程 + 记住队列指针"
//     · 调用契约：**必须先启动队列，再调用 start()**
// ---------------------------------------------------------------------------
